...
\chapter{Results}
Figure \ref{fig:pipeline} illustrates the final GAN search model. A dress image from the dataset is passed to a \textit{Product Synthesis} module, where the dress shape and pattern is modified based on user input. This generated image is passed through another network, \textit{Model Synthesis}, to generate an image of a person wearing the fake dress. Both the generated product image and the generated model image are then passed to the \textit{Product Search} and \textit{Model Search} modules respectively. There feature vectors of the generated images are calculated, and compared to features of original images from the dataset. Based on this comparison, the best (most similar) images are retrieved from the dataset.
...
Surprisingly, all used models train in pixel-space, instead of decoding the images into a latent representation. The pixel-space models, compared with latent encoder models, resulted in more stable and realistic outputs, when trained with the fashion dataset.

The final model enables the user to change the attributes of one product multiple times, e.g: first shorten the sleeves and then round the neckline. However, using a synthesized image as an input for another synthesis can magnify artifacts and return unrealistic outputs. To avoid this behaviour, the user can  change one attribute of the dress, trigger a new image search and select a similar image, and then modify further.

As shown in Figure \ref{fig:consecutive_mod}, the shape modification model does not reconstruct detailed patterns in the images well. Therefore pattern modification should only be performed after modifying the shape of the dress, to avoid loss of the pattern. 
...
The retrieval features are relatively robust when comparing features of generated output against original images. Although, images with a high amount of artifacts or blurry outputs tend to get misinterpreted by the feature.
