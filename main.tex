...
\chapter{Results}
The final GAN retrieval pipeline, illustrated in Figure \ref{fig:pipeline}, consists of 3 basic blocks: data, synthesis and retrieval. 

% Attribute Modification
The final model uses three out of the five evaluated image-to-image translation models: StarGAN for shape modifications, CycleGAN for pattern modifications and Pix2Pix for product-to-model translation. Surprisingly, models that train in pixel-space, instead of decoding the images into a latent representation, resulted in more stable and realistic outputs using the fashion dataset.

Since the shape modification has difficulties keeping the details of the pattern of the dress, the shape should be modified before a pattern modification is applied. It is also possible to stack shape modifications, such as change sleeve length and then change neckline, however, the artifacts created by the first modification are usually magnified. Modifying one image more then 3 times usually results in unrealistic output.

\begin{figure}[h]
\centering
{\includegraphics[width=\linewidth]{05_results/pipeline2}}
\caption{\label{fig:pipeline} \textbf{Fashion GAN Search Pipeline.}}
\end{figure}


Although the model images generated by Pix2Pix network do not look realistic to a human observer, they are sufficient to retrieve similar products photographed on models.  

The tested retrieval features are relatively robust when comparing generated output against original images. The final model uses a combination of a 2048-dimensional ResNet feature and a 50-dimensional low-level feature, using L2 distance as metric for feature comparison.

\begin{figure}[h]
\centering
{\includegraphics[width=\linewidth]{05_results/results_floral}}
\caption{\label{fig:pipeline} \textbf{Fashion GAN Search Pipeline.}}
\end{figure}

\begin{figure}[h]
\centering
{\includegraphics[width=\linewidth]{05_results/floral_product_retrieval}}
{\includegraphics[width=\linewidth]{05_results/floral_model_retrieval}}
\caption{\label{fig:pipeline} \textbf{Fashion GAN Search Pipeline.}}
\end{figure}
