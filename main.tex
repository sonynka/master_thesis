...
\chapter{Results}
The final model generates new product images based on user-specified modifications and triggers a search of existing products that are similar to the generated image. 

As illustrated in Figure \ref{fig:pipeline}, a dress image from the dataset is passed to a \textit{Product Synthesis} module, where the dress shape and pattern is modified based on user input. This generated image is passed through another network, \textit{Model Synthesis}, to generate an image of a person wearing the fake dress. Both the generated product image and the generated model image are then passed to the \textit{Product Search} and \textit{Model Search} modules respectively. The feature vectors of the generated images are calculated, and compared to features of original images from the dataset. Based on this comparison, the best (most similar) images are retrieved from the dataset.
...
\caption{\label{fig:problem_data} \textbf{Examples of similar images from different categories.} \textbf{(a)} Displays dresses with different neckline categories, but the difference is hard to distinguish due to the dark color of the dresses. \textbf{(b)} Displays two fit categories that are supposed to be opposite, but look very similar.}
\end{figure}

Due to the bottleneck architecture of the generator, it is also hard for StarGAN to reconstruct detailed structures in the products, such as patterns. This can be partly fixed by applying a pattern modification trained with CycleGAN image. 

The CycleGAN was trained to translate unicolor images to floral or stripes pattern, and vice versa. While floral translation works well with most of images, the stripes modification output is not as stable. Figure \ref{fig:stripes_retrieval} demonstrates image retrieval with an existing striped image and a generated striped image. Even though the generated striped image is one of the best results of the stripe-translation, the pattern is not clearly recognized by the feature and the retrieval results are mixed with other patterns.
...
After each modification, the generated image is also sent through the product-to-model translation, the Pix2Pix network, which produces a model image from a generated input. This generated model image is supposed to help find more similar products, that are photographed on a person. Although the network does not output realistic images - modelling a person's body and face is too complex for the given networks - the generated images are sufficient to retrieve images with similar products.

\paragraph{Retrieval}
Based on the experiments with different feature vectors, the final model uses a combination of 2048-dimensional ResNet feature and 50-dimensional low-level Akiwi feature, in the ratio of 1:2. The features are combined based on L1 distance.

The retrieval features are relatively robust when comparing features of generated output against original images. Although, images with a high amount of artifacts or blurry outputs tend to get misinterpreted by the feature. Figure demonstrates the retrieval result when using a real striped-dress and a generated striped-dress. Although the generated image is one of the best results of the translation, the retrieved results include other patterns, such as floral and polka dots, as opposed to just striped images.

\begin{figure}[h]
\centering
{\includegraphics[width=\linewidth]{05_results/gan_search}}
\caption{\label{fig:pipeline} \textbf{Fashion GAN Search Results.} Top left image is the original test image, images in the left column underneath are modified versions of the original image. For each image in the left column retrieved images are shown in the respective row. Images were retrieved with Resnet+Akiwi50 feature and L1 distance.}
\end{figure}


\chapter{Conclusion and outlook}

Although the synthesis does not achieve to create new realistic fashion products, the generated outputs are sufficient to guide the image retrieval and retrieve more similar images. 

This type of synthesis can be useful, to for example browse a fashion dataset that does not have options for search based on textual attributes. 

However, training of the models and the options that the final model will offer to the user, is highly dependent on the available data. Adding more attributes for the user to modify, improving the training or the retrieval results - it is all a question of whether this data is available.
